...
def plots():
    ##########Plotting average and original Jacobian##########
    ...
    ##########Calculating diffference and percent difference##########
    (r, c) = avgJac.shape
    diff = []
    ...
    for i in xrange(r):
        for j in xrange(c):
            difference = avgJac[i,j] - originalJac[i,j]
            if originalJac[i,j] != 0:
                 percent = abs((difference/originalJac[i,j]))*100
            else:
                 percent = abs(difference)*100
            percent_difference.append(percent)
    percent_difference = np.asarray(percent_difference)
    percent_difference.shape = (r,c)
    print np.amax(percent_difference)
    print 'Creating figure'
    pfig = plt.figure()
    px = pfig.add_subplot(111, title='% Difference')
    im_p = px.pcolormesh(percent_difference, cmap = 'Blues')
    pfig.colorbar(im_p)
    ...
    (x,y) = percent_difference.shape
    for i in xrange(x):
        for j in xrange(y):
            t.write('%s '%percent_difference[i,j])
            if percent_difference[i,j] > args.filter:
                f.write('(%s,%s)'%(i,j))
                f.write(' ')
                f.write(str(percent_difference[i,j]))
                f.write('\n')
                percent_difference[i,j]=0
    os.chdir('%s/plots'%(cwd))
    ffig = plt.figure()
    fx = ffig.add_subplot(111,title='Filtered % Difference')
    im_f = fx.pcolormesh(percent_difference, cmap='Blues')
    ffig.colorbar(im_f)
    ffig.savefig('%_diff_filtered'+str(args.filter)+'.png')
...
def tests():
    ...
    for i in xrange(r):
        for j in xrange(c):
            difference = stddev[i,j] - avgJac[i,j]
            if avgJac[i,j] != 0:
                 percent = abs((difference/avgJac[i,j]))
            else:
    ...
    im_ps = psx.pcolormesh(percent_difference, cmap = 'Blues')
    ...
    filtered_percent_difference = percent_difference
    (x,y) = filter_percent_difference.shape
    for i in xrange(x):
        for j in xrange(y):
            if filtered_percent_difference[i,j] > 10:
                s.write('(%s,%s) '%(i,j))
                s.write(str(filtered_percent_difference[i,j]))
                s.write('\n')
                filtered_percent_difference[i,j]=0
    os.chdir('%s/plots'%(cwd))
    sfig = plt.figure()
    sx = sfig.add_subplot(111,title='Filtered STD % Difference')
    im_s = sx.pcolormesh(filtered_percent_difference, cmap='Blues')
    sfig.colorbar(im_s)
